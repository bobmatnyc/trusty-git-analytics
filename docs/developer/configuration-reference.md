...
| `llm_fallback_scope` | enum | no | `low_confidence` | Which verdicts reach the LLM: `low_confidence` (confidence at or below `llm_fallback_threshold`) or `unanswered` (only commits the rules left uncategorized; the threshold is ignored). Merge commits are never sent to the LLM, under either scope. #111 |

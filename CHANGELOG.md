...
- Added the Bedrock Claude Sonnet 5 measurement to the classification
  accuracy and cost-benefit report at `docs/classification/jev/`, replacing
  the earlier "not yet measured" placeholders (#111).
...
- The weekly report and `weekly_activity.csv` gain `ai_trailer_count`,
  ...
  by that same signal family (trusty-tools#4418). Appended after the existing columns, so a
...
- `tga audit` reads its lookback window from a new `audit.window_weeks` config
  ...
  it — collected unbounded history (trusty-tools#5482).
- `tga aliases suggest --review-file <PATH>` writes the near-miss identity pairs —
  ...
  the flag is given (trusty-tools#6993).
...
- `compute_dora()` now reads `fact_deployments` (`environment='production',
  ...
  (trusty-tools#212). A repo with real production deploy history previously reported
...
- `tga pr-metrics` no longer returns success on a header-only CSV. The artifact is
  ...
  it: an empty `pull_requests` table (pointing at `github.fetch_prs`, trusty-tools#211, and a
  missing non-interactive git credential, trusty-tools#6244), a lookback window that excluded
  ...
  reaches the report's Gaps & Caveats without aborting the sweep (trusty-tools#6796).
- A pull request whose author login is empty — GitHub's answer for a deleted
  ...
  no longer vanishes from the opened, merged, and cycle-time totals (trusty-tools#6796).
...
- `tga jira sync` posts to `/rest/api/3/search/jql`. Atlassian removed
  ...
  and never on page length (trusty-tools#6812).
...
- `tga audit`'s report now names the "GitHub pull requests" collection leg as
  ...
  against repositories whose `origin` remote is GitHub-hosted (trusty-tools#7132). Before
...
- `tga audit`'s Gaps & Caveats data-handling line now runs the real
  data-retention attestation trusty-tools#5218 shipped, instead of unconditionally
  quoting the pre-trusty-tools#5218 placeholder saying an attestation is "pending
  (trusty-tools#5218)" (trusty-tools#7140). Every DD report generated after `tga` 7.1.0 carried that
  stale sentence even though trusty-tools#5218 had already shipped. The line now states a
...
- `tga audit` passes `--allow-degraded` to `trusty-review report`, so a run whose
  ...
  rejects an unknown flag with a usage error (trusty-tools#6811).
...
- Identity resolution no longer runs the Tier-3/4 Jaro-Winkler fuzzy fallback
  ...
  distinct people onto similarly-spelled colleagues — `Crispian Alvarenga` →
  `Crisandra Fonseca`, `Nico Vanderbergheim` → `Nico Mercer`, `Marci Dalton` →
  `Marcel Sutton`, `Josh Dorsey` and `Joseph Wu` → `Joshua Kepler` — none of

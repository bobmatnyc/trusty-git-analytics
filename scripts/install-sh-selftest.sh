#!/usr/bin/env bash
# Selftest for ./install.sh, the trusty-audit installer (trusty-tools#5870).
#
# Ported from trusty-tools scripts/check_trusty_audit_install_selftest.sh
...
#
# Usage: scripts/install-sh-selftest.sh
...
# Every URL the stub curl was asked for, one per line (case 7 reads it).
...
# ── Case 7 — every URL the happy path fetched names this repository ─────────
# The installer moved from bobmatnyc/trusty-tools; a leftover REPO would
# resolve the other repository's releases and still exit 0 in a stub run.
if [[ "$(wc -l <"${URL_LOG}" | tr -d ' ')" == "3" ]] &&
    ! grep -v -e '^https://api.github.com/repos/bobmatnyc/trusty-git-analytics/releases' \
        -e '^https://github.com/bobmatnyc/trusty-git-analytics/releases/download/trusty-audit-v' \
        "${URL_LOG}" >/dev/null; then
    pass "happy path fetched only bobmatnyc/trusty-git-analytics URLs"
else
    fail "happy path URLs: $(cat "${URL_LOG}")"
fi

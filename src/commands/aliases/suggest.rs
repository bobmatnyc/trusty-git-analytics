...
///   `researhco.example` vs `researchco.example` (HIGH on dist ≤ 2).
///
...
mod tests {
    ...
    fn dotlocal_email_routed_to_canonical_domain() {
        ...
        let out = detect_noise_patterns(&db, Some("researchco.example")).expect("detect");
        assert!(
    ...
    fn domain_typo_detected_against_canonical_domain() {
        let db = Database::open_in_memory().expect("open");
        insert_author(&db, "Carol", "carol@researhco.example"); // typo
        insert_author(&db, "Carol", "carol@researchco.example");
        let out = detect_noise_patterns(&db, Some("researchco.example")).expect("detect");
        assert!(
    ...
    fn config_canonical_domain_threads_through() {
        ...
        let mut db = Database::open_in_memory().expect("open");
        insert_author(&db, "Z", "z@researhco.example");
        insert_author(&db, "Z", "z@researchco.example");
        let cfg = config_with_domain("researchco.example");

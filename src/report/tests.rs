...
//!
//! Why: keeps the test suite co-located with the production code it exercises
//! while staying in a dedicated file so the 1 500-SLOC test cap applies
//! instead of the 500-SLOC production cap on `mod.rs`.
//! What: end-to-end tests that drive `Aggregator`, formatters, and the pipeline
//! with seeded in-memory SQLite databases.

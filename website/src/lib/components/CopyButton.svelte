<script lang="ts">
	/**
	 * Why: an install command is meant to land in a terminal exactly as shown.
	 * Selecting it by hand risks catching a stray character, and a mouse-only
	 * "select the `<pre>`" affordance leaves keyboard and screen-reader users
	 * with no equivalent shortcut.
	 *
	 ...
	 * rejected; both land in `error`, never a false `copied` — the source
	 * `<pre>` is never touched, so its text stays selectable either way.
	 */
	interface Props {

 ...
 * toggle component — is what makes it testable without mounting anything,
 * and lets the pre-paint snippet in `app.html` reuse the same key and the
 * same `.dark` class without importing the module graph.
 *
 ...
 * `darkMode: 'class'` and `app.css`'s `.dark` token block both key off.
 *
 * Every function is a no-op (or returns the default) when `window` is
 * absent, because every route here is prerendered and therefore runs this
 * module in Node at build time.
 *
